...
class MovieBot(Client):
    ...
    async def start(self):
        ...
        try:
            ...
            self.movies = None # Explicitly set to None if failed
...
async def stats_cmd(client, msg):
    if client.movies is None:
        return await msg.reply("⚠️ **Error:** Database Connect Nahi Hua!\nMongoDB URL check karein.")
...
async def add_to_db(client, msg):
    if client.movies is None:
        return # DB nahi hai to kuch mat karo

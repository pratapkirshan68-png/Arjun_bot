...
# ================= ADS + WATERMARK =================
ADS_TEXT = (
    "🔥 **Movies 2026 – Cinema Pratap** 🔥\n\n"
    "🎬 Latest Movies Daily\n"
    "❤️ Support by Joining Channel\n\n"
)

WATERMARK_TEXT = (
    ADS_TEXT +
    "⚠️ Ye movie **2 minute** me auto delete ho jayegi!"
)
...
async def add_movie(_, msg):
    ...
    r = await msg.reply(f"✅ **Movie Added:** `{title}`")
    asyncio.create_task(auto_delete(r, 60))
...
async def search(_, msg):
    ...
    if not res:
        m = await msg.reply("❌ Movie nahi mili")
...
async def wrong_place(_, msg):
    ...
    r = await msg.reply(
        ...
        "👉 Movie search ke liye niche group par jao.",
        reply_markup=btn
    )
    asyncio.create_task(auto_delete(r, 60))
...
async def start(_, msg):
    ...
    if len(msg.command) > 1 and msg.command[1].startswith("file_"):
        ...
        r = await msg.reply("👋 Bot ready hai, search group me movie naam likho")
        asyncio.create_task(auto_delete(r, 60))

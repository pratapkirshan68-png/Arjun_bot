...
# BOT 1: Movie Bot Token
BOT_TOKEN = get_clean_var("BOT_TOKEN", "")

# BOT 2: Poster Changer Bot Token
THUMB_BOT_TOKEN = get_clean_var("THUMB_BOT_TOKEN", "")

MONGO_URL = get_clean_var("MONGO_URL", "")
TMDB_API_KEY = get_clean_var("TMDB_API_KEY", "")
GEMINI_API_KEY = get_clean_var("GEMINI_API_KEY", "")
...
class MovieBot(Client):
    ...
    async def start(self):
        ...
        print(f"🚀 MOVIE BOT STARTED as @{self.me.username}")

app = MovieBot()

# ================= BOT 2 CLIENT (Poster Changer Bot) =================
app_thumb = Client(
    "poster_changer_session",
    api_id=API_ID,
    api_hash=API_HASH,
    bot_token=THUMB_BOT_TOKEN if THUMB_BOT_TOKEN else BOT_TOKEN
)
...
async def start_bot():
    ...
    app_web.router.add_get("/", lambda r: web.Response(text="Both Bots Alive"))
    runner = web.AppRunner(app_web)
    await runner.setup()
    await web.TCPSite(runner, "0.0.0.0", int(os.environ.get("PORT", 8080))).start()
    
    await app.start()
    await app_thumb.start()
    print("✅ **Dono Bots Successfully Render Par Active Hain!**")
    
    await idle()
    
    await app.stop()
    await app_thumb.stop()

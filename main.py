...
async def search_movie(client, msg):
    ...
    btn = InlineKeyboardMarkup([
        [InlineKeyboardButton("🚀 DOWNLOAD / WATCH NOW", url=final_link)],
    ...
    await sm.delete()
    asyncio.create_task(delete_after_delay([res_msg], 120)) & filters.text & ~filters.command(["start", "pratap", "shortlink", "del"]))
async def search_movie(client, msg):
    query = clean_name(msg.text)

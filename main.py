...
# IDs ko integer me convert karna zaruri hai
try:
    ...
    print("❌ Error: Channel IDs numbers hone chahiye (Ex: -10012345678)")
...
try:
    # Connection timeout badha diya hai aur DNS fix lagaya hai
...
async def add_movie(client, msg):
    print(f"📥 New File Detected in Channel: {msg.chat.id}")
...
async def search(client, msg):
    ...
    print(f"🔍 Searching: {query} in Chat: {msg.chat.id}")
...
@bot.on_message(filters.command(["pratap", "pratap.movie", "stats"]) & filters.user(ADMIN_IDS))
async def stats(client, msg):
    try:
        c = await movies.count_documents({})
        await msg.reply(f"📊 **Total Movies:** {c}\n✅ DB Connected")
    except Exception as e:
        await msg.reply(f"❌ Database Error: {e}")
...
async def start(client, msg):
    ...
    # FSUB Check
    try:
        await client.get_chat_member(FSUB_CHANNEL, uid)
    ...
    if len(msg.command) > 1 and msg.command[1].startswith("file_"):
        ...
        try:
            ...
            if res:
                ...
                await msg.reply("❌ Movie exist nahi karti.")
        except:
...
async def main():
    ...
    try:
        c = await movies.count_documents({})
        print(f"✅ DB Connected. Total Movies: {c}")
    except Exception as e:
        print(f"❌ FATAL ERROR: Database connect nahi hua!\nReason: {e}")

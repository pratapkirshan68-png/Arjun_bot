...
async def search_movie(client, msg):
    ...
    if not movie:
        await msg.reply(f"😔 **{msg.text}** nahi mili bhai")
        return
    ...
    start_link = f"https://t.me/{me.username}?start=file_{movie['_id']}"
    short_url = await shortlink(start_link)

    # 🔘 Button
    buttons = InlineKeyboardMarkup([
        [InlineKeyboardButton("🚀 GET MOVIE", url=short_url)]
    ])
    ...
    caption = (
        ...
        f"🆔 `{msg.from_user.id}`\n\n"
        f"😔 Search working bhai..."
    )

    # 🖼 Poster small (TMDB w500 = already small)
    if poster:
...
async def short_toggle(_, msg):
    ...
    await msg.reply("✅ Shortlink Updated")

...
class MovieBot(Client):
    def __init__(self):
        ...
        self.db = None
        self.movies = None
        self.db_error = None  # Error store karne ke liye

    async def start(self):
        await super().start()
        # MongoDB Connection with SSL Fix
        try:
            # Ye line sabse important hai - tlsAllowInvalidCertificates=True
            self.mongo_client = AsyncIOMotorClient(MONGO_URL, tls=True, tlsAllowInvalidCertificates=True)
            ...
            self.db_error = str(e) # Error ko save kar lenge
            self.movies = None 

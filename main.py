...
async def get_search_buttons(query, results, offset=0):
    ...
    # Base64 encoding taaki link na toote
...
async def start_handler(client, msg):
    # Data nikal rahe hain (file ID ya search query)
    ...
    try:
        ...
        # Agar koi file link hai, toh "Try Again" button dikhao
        if data:
        ...
        return await msg.reply(
            "❌ **Access Denied!**\n\nFile lene ke liye pehle hamare channel ko join karein, phir niche diye gaye **Try Again** button par click karein.",
            reply_markup=btn
        )
    except Exception:
        pass

    # --- Agar user member hai, toh niche ka logic chalega ---
    
    if not data: 
        return await msg.reply("👋 Namaste! Group me search karein.")

    # --- Ek single file bhejne ka logic ---
    if data.startswith("file_"):
        file_id_db = data.split("_")[1]
        res = await client.movies.find_one({"_id": ObjectId(file_id_db)})
        if res:
    ...
    # --- GET ALL FILES button ka logic ---
        ...
        await sts.edit(f"✅ **Batch Complete!** Total `{count}` files bheji gayi.")

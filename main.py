...
async def search_movie(client, msg):
    ...
    if not results:
        ...
        for admin_id in ADMIN_IDS:
            try: await client.send_message(admin_id, admin_alert)
            except: pass
...
async def toggle_shortlink_cmd(client, msg):
    ...
    if choice == "on": SHORTLINK_ENABLED = True; await msg.reply("✅ Shortlink ON")
    elif choice == "off": SHORTLINK_ENABLED = False; await msg.reply("❌ Shortlink OFF")
...
if __name__ == "__main__":
    try:
        ...
        print("Bot Stopped Manual.")        url = f"https://api.themoviedb.org/3/search/multi?api_key={TMDB_API_KEY}&query={quote(query)}"
        async with aiohttp.ClientSession() as session:
            async with session.get(url, timeout=10) as resp:
                if resp.status == 200:
                    data = await resp.json()
...
async def search_movie(client, msg):
    ...
    # Detect if message is from Admin or Channel Admin
    is_admin = False
    if user and user.id in ADMIN_IDS:
        is_admin = True
    elif msg.sender_chat and msg.sender_chat.id in [SEARCH_CHAT, STORAGE_CHANNEL]:
        is_admin = True

    user_id = user.id if user else (msg.sender_chat.id if msg.sender_chat else msg.chat.id)
    user_name = user.first_name if user else (msg.sender_chat.title if msg.sender_chat else "User")

    # Save real user to DB
    if user:
        await add_user_to_db(user)
    ...
    if not results:
        ...
        # Save request to MongoDB (Only for real users)
        if user_id > 0:
            try:
                await client.requests.update_one(
                    {"clean_name": query},
...
# ================= STORAGE CHANNEL (NEW MOVIE ADDED) =================
...
# 1. BroadCast / SMS Command to All Users
@app.on_message(filters.command(["broadcast", "sms"]) & filters.user(ADMIN_IDS))
async def broadcast_command(client, msg):
    if not msg.reply_to_message and len(msg.command) < 2:
        return await msg.reply("Usage:\n`/broadcast <Your Message>` OR reply to a message with `/broadcast`")
...
# 2. Admin Movie Info / TMDB Details Command
@app.on_message(filters.command(["details", "tmdb"]) & filters.user(ADMIN_IDS))
async def tmdb_details_cmd(client, msg):
    if len(msg.command) < 2:
        return await msg.reply("Usage: `/details <movie name>` (e.g. `/details Jawan`)")
    ...
    text = (
        f"🎬 **MOVIE DETAILS (AI / TMDB)**\n\n"
        f"📌 **Title        url = f"https://api.themoviedb.org/3/search/multi?api_key={TMDB_API_KEY}&query={quote(query)}"
        async with aiohttp.ClientSession() as session:
            async with session.get(url, timeout=10) as resp:
                if resp.status == 200:
                    data = await resp.json()
...
@app.on_message(filters.chat(SEARCH_CHAT) & filters.text & ~filters.command(["start", "pratap", "delall", "del", "shortlink", "broadcast", "sms", "details", "tmdb"]))
async def search_movie(client, msg):
    # Rule 5: Ignore Admin Messages from auto-deletion or filtering
    ...
    if not results:
        ...
        await client.requests.update_one(
            {"query": query},
            {
                "$set": {"movie_name": msg.text, "clean_name": query, "tmdb": tmdb_info},
                "$addToSet": {"requested_users": msg.from_user.id},
                "$setOnInsert": {"created_at": datetime.utcnow()}
            },
            upsert=True
        )

        # Case 1: Upcoming / Unreleased Movie on TMDB
        if tmdb_info and tmdb_info.get("status") == "Upcoming":
            rel_date = tmdb_info.get("release_date", "N/A")
            days_left = tmdb_info.get("days_remaining", 0)
            text = (
                f"ℹ️ **MOVIE RELEASE DETAILS**\n\n"
                f"👤 **User:** {user_mention} (`{msg.from_user.id}`)\n"
                f"🎬 **Movie:** `{tmdb_info['title']}`\n"
                f"📅 **Release Date:** `{rel_date}`\n"
                f"⏳ **Status:** Abhi release nahi hui (Releasing in `{days_left}` days)\n\n"
                f"📝 Movie release hote hi hamare database me add kar di jayegi!"
        ...
        # Case 2: General missing movie response
        else:
            text = (
                f"Maaf karna {user_mention}, abhi hamare database me **'{msg.text}'** movie nahi hai. 😔\n\n"
                f"✅ Humne aapki request Admin ko bhej di hai.\n"
                f"📲 Jaise hi movie add ho jayegi, aapko personal message (SMS) mil jayega!"
            )
            res_msg = await client.send_message(msg.chat.id, text=text)

        # Admin & Main Channel Notification
        admin_alert = (
            f"📥 **NEW MOVIE REQUEST**\n\n"
        ...
        # Admins ko notification
        for admin_id in ADMIN_IDS:
            try: await client.send_message(admin_id, admin_alert)
            except: pass

        # Main Channel me Alert Message
        if MAIN_CHANNEL_LINK:
            try:
                # Group/Channel notification logic if channel ID configured
                await client.send_message(SEARCH_CHAT, f"📢 **Admin Notification Sent!** Movie `{msg.text}` will be added soon.")
            except: pass
...
# 3. New Movie Added to DB Channel Handler
@app.on_message(filters.chat(STORAGE_CHANNEL) & (filters.video | filters.document))
async def add_to_db(client, msg):
    file = msg.video or msg.document
    original_title = msg.caption or file.file_name or "Unknown"
    ...
    req_cursor = client.requests.find({"clean_name": {"$regex": search_title, "$options": "i"}})
    requests = await req_cursor.to_list(length=100)
            ...
            if user_id not in notified_users:
                try:
                    alert_msg = (
                        f"🎉 **Movie Available Now!**\n\n"
                        f"Aapne jis movie (`{original_title}`) ke liye request ki thi, wo ab hamare database me add ho chuki hai!\n\n"
                        f"👉 Search Group me jaakar dobara search karein aur download karein!"
                    )
                    await client.send_message(user_id, alert_msg)
                    notified_users.add(user_id)
                except: pass

    # Optionally Notify ALL Bot Users (SMS/Broadcast alert)
    users_cursor = client.users.find({})
        ...
        if uid not in notified_users:
            try:
                await client.send_message(
                    uid,
                    f"🔥 **NEW MOVIE ADDED!**\n\n"
                    f"🎬 `{original_title}` ab available hai!\n"
                    f"👉 Abhi Search Group me jaakar download karein."
                )
                await asyncio.sleep(0.05)
            except: pass

# 4. Existing Admin Commands
@app.on_message(filters.command("shortlink") & filters.user(ADMIN_IDS))
async def toggle_shortlink_cmd(client, msg):
    global SHORTLINK_ENABLED
    choice = msg.command[1].lower() if len(msg.command) > 1 else ""

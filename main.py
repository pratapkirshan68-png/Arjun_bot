...
async def search_movie(client, msg):
    ...
    markup = await async def get_search_buttons(query, results, offset=0):
    btn_list = []
    me = await app.get_me()
    for res in results[offset : offset + PAGE_SIZE]:
        db_id = str(res["_id"])
    ...
    # Yahan humne query ko encode kiya hai taaki link na toote
    query_encoded = base64.urlsafe_b64encode(query.encode()).decode().rstrip("=")
    btn_list.append([InlineKeyboardButton("📂 GET ALL FILES (IN PM) 📂", url=f"https://t.me/{me.username}?start=all_{query_encoded}")])
    return InlineKeyboardMarkup(btn_list)
...
async def start_handler(client, msg):
    ...
    if data.startswith("file_"):
        ...
        # Yahan hum encode ki hui query ko wapas text banayenge
        try:
            encoded_str = data.split("_", 1)[1]
            encoded_str += "=" * ((4 - len(encoded_str) % 4) % 4) # Padding fix
            search_q = base64.urlsafe_b64decode(encoded_str).decode()
        except:
            search_q = unquote(data.split("_", 1)[1])

        # Search logic (Same as Group)
        cursor = client.movies.find({"title": {"$regex": search_q, "$options": "i"}})
        results = await cursor.to_list(length=100)
        ...
        # Ab ye message PM mein dikhna chahiye
        sts = await msg.reply(f"🚀 **Found {len(results)} files.** Sending to you...")

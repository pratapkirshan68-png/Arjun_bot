...
# Official Google Gen AI Client Setup
...
# ================= FEATURE 7: AI CHAT (ADMIN PM ONLY - DYNAMIC MODEL SEARCH) =================
def fetch_active_model_name() -> str:
    """API key dwara accessible active stable text generation model name search karta hai."""
    if not ai_client:
        return ""
    try:
        # Fetch models dynamically to avoid 404 NOT_FOUND errors
        ...
        # Fallback to standard supported format if iteration yields empty
...
def generate_ai_text_single_request(prompt: str) -> str:
    """Hardcoding, retry loops aur quota spam ke bina single API request bhejta hai."""
    if not ai_client:
        return "⚠️ GEMINI_API_KEY set nahi hai."

    target_model = fetch_active_model_name()
    if not target_model:
        return "⚠️ Koi active AI model nahi mila."
    ...
    try:
        # STRICT SINGLE API CALL - No retry loops
        ...
        if error_code == 429 or "RESOURCE_EXHAUSTED" in error_msg or "Quota exceeded" in error_msg:
            return "⚠️ AI API Quota Exceeded! Kripya kuch samay baad try karein ya Gemini API Key renew karein."
        elif error_code == 404 or "NOT_FOUND" in error_msg:
            return f"⚠️ Requested AI Model (`{target_model}`) available nahi hai."
        else:
...
async def toggle_shortlink_cmd(client, msg):    if not TMDB_API_KEY: return None
    try:
        url = f"https://api.themoviedb.org/3/search/multi?api_key={TMDB_API_KEY}&query={quote(query)}"
        async with aiohttp.ClientSession() as session:
            async with session.get(url) as resp:
...
# ================= FEATURE 7: AI CHAT (ADMIN PM ONLY - OFFICIAL GEMINI SDK) =================
def generate_ai_text(prompt: str) -> str:
    """Latest Official Google Gen AI SDK ke dwara AI content generate karta hai."""
    if not ai_client:
        return "⚠️ GEMINI_API_KEY set nahi hai."
    ...
    return "⚠️ AI Service response nahi de pa rahi hai. Kripya kuch samay baad try karein."
...
async def ai_chat_handler(client, msg):
    ...
    try:
        # Non-blocking async execution
        ai_reply = await asyncio.to_thread(generate_ai_text, prompt)

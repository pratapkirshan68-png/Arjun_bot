...
async def start_handler(client, msg):
    # FSUB Check (Channel join check karne ke liye)
    try:
    ...
    # Ek single file bhejne ke liye logic
    ...
    # GET ALL FILES button ka logic (Yahan fix kiya gaya hai)
    elif data.startswith("all_"):
        search_q = unquote(data.split("_", 1)[1])
        
        # 1. Pehle exact name search karein
        cursor = client.movies.find({"title": {"$regex": search_q, "$options": "i"}})
        results = await cursor.to_list(length=100)

        # 2. Agar exact nahi mila, toh group wala Spelling Correction logic use karein
        if not results:
        ...
        sts = await msg.reply(f"🚀 **Sending {len(results)} files...**\nKripya thoda intezar karein.")
        ...
        for res in results:
            try:
                ...
                count += 1
                await asyncio.sleep(1.5) # Telegram limit se bachne ke liye delay
            except Exception as e:
                logger.error(f"Error sending file: {e}")

        await sts.edit(f"✅ **Batch Complete!**\nTotal `{count}` files bheji gayi hain.")
